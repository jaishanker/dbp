...
class ActivityStream < ActiveRecord::Base
  ...
  def self.paginate_recent_actors(actor, location , offset, limit=PER_PAGE)
    unless actor.class.name == ACTIVITY_STREAM_USER_MODEL
      find(:all, :conditions => 
        {:actor_id => actor.id, 
        :actor_type => actor.class.name, 
        :status => 0}, :order => "created_at DESC", :limit => limit,:offset =>offset,
        :include => [:actor, :object, :indirect_object])
    else
      # FIXME: We really want :include => [:actor, :object], however, when
      # the "p.id" => nil condition prevents polymorphic :include from working
  ...
  def self.latest_activity(user_id, location)
    # FIXME: We really want :include => [:actor, :object], however, when
    # the "p.id" => nil condition prevents polymorphic :include from working
    find(:all, 
      :joins => self.preference_join(location),
      :conditions => [
          "indirect_object_id = ? and actor_id <> ? and object_type NOT IN ('Learning','Game','News') and status = ? and p.id IS NULL", 
          user_id,user_id, 0 ],      
#      :conditions => condition_string,
  ...
  def self.paginate_recent_objects(object, location,offset, limit=PER_PAGE)
    # FIXME: We really want :include => [:actor, :object], however, when
    # the "p.id" => nil condition prevents polymorphic :include from working
    find(:all, 
      :joins => self.preference_join(location),
  ...
  def self.paginate_recent_activities(offset, limit=PER_PAGE)
#    unless actor.class.name == ACTIVITY_STREAM_USER_MODEL
      find(:all, :conditions => 
          {:status => 0}, :order => "created_at DESC", :limit => limit,:offset =>offset,
        :include => [:actor, :object, :indirect_object])
...
#    end  
  ...
  def self.specific_activities(object_type, location,offset=0, limit=PER_PAGE)
    find(:all, 
      :joins => self.preference_join(location),
      :conditions => [
          "object_type = ? and status = ? and p.id IS NULL", 
          object_type, 0 ],
      :order => "created_at DESC",
      :limit => limit,
      :offset => offset)
