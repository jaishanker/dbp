...
#    end
  ...
  def write_activity_stream_log(actor_method, actor_name, verb, object_method,
    ...
    if verb == "commented on" or verb == "replied to" or verb== "posted reply on"
      indirect_object_phrase = self.instance_variable_get(options[:indirect_object_phrase])
    end
    ...
    actors.each do |actor|
      objects.each do |object|
        ...
        # Modified by anil for verb to have his/her according to user gender
        if verb.to_s.include?("his")
            if actor.gender == "Female"
               verb = verb.to_s.gsub('his','her')
            end
        end               
        # If there are identical activities within 8 hours, up count  
         if activity == :profile_updater        
        ...
        if activity_stream
          ...
          unless activity == :rated_design and actor ==  object.user
                ...
                if indirect_object
                  ...
                  if verb == "commented on" or verb == "replied to" or verb== "posted reply on"
                      activity_stream.indirect_object_phrase = indirect_object_phrase
                  else
