...
#            favorite_class.paginate :page => page, :include => :favorites,
...
#                                                id, favorite_class.to_s ]            
            ...
            favorite_class = ($1).singularize.camelize.constantize
            Favorite.count( :include => :user, :conditions => [ 'favorites.user_id = ? AND favorites.favorable_type = ? AND favorites.status = 1', 
